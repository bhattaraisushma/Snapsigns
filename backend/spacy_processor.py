...
# important_words = {"i", "we", "me", "you", "he", "she", "it", "they", "them", "us", "why", "what", "how", "who", "where", "when", "which"}

# def process_text(input_text):
    
#     doc = nlp(input_text)

#     filtered_tokens = [token.lemma_.upper() for token in doc if token.text.lower() in important_words or (not token.is_stop and not token.is_punct)]
...
#         "pos_tags": [token.pos_.upper() for token in doc if (not token.is_stop and not token.is_punct) or token.text.lower() in important_words],
#         "asl_gloss": asl_gloss
#     }
...
VERB_EXCEPTIONS = {"singing": "sing", "running": "run", "dubbing": "dub"}
...
def process_text(input_text: str) -> dict:
    ...
    processed_data = {
        ...
        "pos_tags": [
            token.pos_.upper()
            for token in doc
            if token.text.lower() in IMPORTANT_WORDS or (not token.is_stop and not token.is_punct)
        ],
        "asl_gloss": asl_gloss
    }
